...
import { getActivityReviews } from '@/fetches/activitiesReviews';
import ReviewPagination from './pagination/Pagination';
import noReview from '@/images/empty.svg';

export default async function ActivityReviews({ params }: { params: { id: string } }) {
  const activity = await getActivityId({ id: Number(params.id) });
  const reviews = await getActivityReviews({ activityId: Number(params.id) });
